...
class InstallerController extends ThinkUpController {
    ...
    public function __construct() {
        ...
        $this->__installer = Installer::getInstance($this);
 ...
 */  
    private function __step1() {
        // php version check
        $php_compat = 0;
        if ( $this->__installer->checkVersion() ) {
            $php_compat = 1;
        }
        $this->addToView('php_compat', $php_compat);
        $requiredVersion = $this->__installer->getRequiredVersion();
        $this->addToView('php_required_version', $requiredVersion['php']);
        
        // libs check
        $libs = $this->__installer->checkDependency();
        $libs_compat = TRUE;
        foreach ($libs as $lib) {
            if (!$lib) {
                $libs_compat = FALSE;
        ...
        $permissions = $this->__installer->checkPermission();
 ...
 */  
    private function __step2() {
        ...
        if ( !$this->__installer->checkStep1() ) {
 ...
 */  
    private function __step3() {
        ...
        // check db
        try {
            $this->__installer->checkDb($db_config);
        } catch (InstallerError $e) {
            return $e->showError($this);
        }
        
        // check email
        if ( !$this->__installer->checkValidEmail($email) ) {
        ...
        $admin_user = array('email' => $email, 'password' => $password);
        // trying to create config file
        $this->__installer->createConfigFile($db_config, $admin_user);
        
        // check tables
        try {
            $this->__installer->checkTable($db_config);
        } catch (InstallerError $e) {
        ...
        $this->__installer->populateTables($db_config);
        
        global $THINKUP_CFG;
        $THINKUP_CFG = $db_config;
        $admin_exists = $this->__installer->isAdminExists();
      
        if ( !$admin_exists ) { // create admin if not exists
            $this->__installer->insertAdmin($db_config['table_prefix'] . 'owners', $admin_user);
        ...
        $this->addToView('errors', $this->__installer->getErrorMessages() );
 ...
 */    
    function repairPage($params = null) {
        ...
        try {
            $this->__installer->repairerCheckStep1();
        } catch (InstallerError $e) {
            $e->showError($this);
        }
        ...
        try {
            $config_file = $this->__installer->repairerCheckConfigFile();
        } catch (InstallerError $e) {
            $e->showError($this);
        }
        require $config_file;
        ...
        // check database
        try {
            $this->__installer->checkDb($THINKUP_CFG);
        } catch (InstallerError $e) {
            $e->showError($this);
        }
        ...
        try {
            $this->__installer->repairerIsDefined($THINKUP_CFG);
        } catch (InstallerError $e) {
            $e->showError($this);
        }
        
        // clearing error messages before doing the repair
        $error_messages = $this->__installer->getErrorMessages();
        if ( !empty($error_messages) ) {
            $this->__installer->clearErrorMessages();
        }
        ...
        if ( isset($_POST['repair']) && !empty($_GET) ) {
            ...
            if ( isset($params['db']) ) {
                $messages['db'] = $this->__installer->repairTables($THINKUP_CFG);
            ...
            if ( isset($params['admin']) ) {
                ...
                $this->__installer->insertAdmin($THINKUP_CFG['table_prefix'] . 'owners', $insertAdminVal);
        ...
        if ( $page == 'index' ) {
            if ( file_exists( THINKUP_WEBAPP_PATH . 'config.inc.php' ) ) {
                ...
                try {
                    ...
                    if ( $this->__installer->isThinkTankInstalled($THINKUP_CFG) && 
                         $this->__installer->checkPath($THINKUP_CFG) )
                    {
